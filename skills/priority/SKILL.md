...
Accept a code, color, or Chinese priority name. Then use the Codex task-title/thread-title tool to prefix the current title with the matching circle. P0-P5 replace the existing P0-P5 circle while preserving a following `⚪` pause marker. P6 adds one `⚪` without removing P0-P5. Preserve the rest of the title, never duplicate markers, and continue the user's task.
